        ...
        <Modal isOpen={isOpen} onClose={onClose} isCentered motionPreset="scale">
            ...
            <ModalContent borderRadius="lg" boxShadow="lg" p={4}>
                <form onSubmit={handleSubmit(onSubmitForm)}>
                    ...
                    <ModalCloseButton />
                    <ModalBody>
                        {fields.fields1.map((field) => (
                            <FormControl key={field.name} mb={4} isInvalid={errors[field.name]}>
                                <FormLabel fontWeight="medium">{field.label}</FormLabel>
                                {field.type === "select" ? (
                                    <Select {...register(field.name, { required: "This field is required" })}>
                                            ...
                                            pattern:
                                                field.name === "email"
                                                    ...
                                                    : field.name === "phoneNumber"
                                                        ? { value: /^0\d{9}$/, message: "Phone number must start with 0 and have 10 digits" }
                                                        : undefined,
                                        })}
                                    />
                                )}
                    ...
                    <ModalFooter display="flex" justifyContent="center" gap={3}>
                        <Button colorScheme="orange" type="submit" isLoading={isSubmitting} size="lg" px={6}>
                            {fields.submitText}
                        </Button>
                        <Button onClick={onClose} size="lg" px={6} variant="outline" colorScheme="gray">
                            Cancel
                        </Button>
                    </ModalFooter>
                </form>
